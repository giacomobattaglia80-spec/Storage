...
OUT = "/home/user/Storage/Hennecke/pitch/kpi_framework_slide.pptx"
...
para(tb(50,28,1200,40),"KPI framework — crescita, redditività & cassa",size=26,color=BLUE,bold=True,first=True)
para(tb(50,76,1200,26),"Leve Commercial & Operations alla base del monitoraggio del business della target",size=13,color=GRAY,first=True)
para(tb(1150,26,400,22),"CONFIDENTIAL · For Board discussion",size=9.5,color=MUTED,align="r",first=True)

# column headers
for x,txt,col in [(210,"CRESCITA (Growth)",GREEN),(670,"REDDITIVITÀ (Profitability)",BLUE),(1130,"CASSA (Cash)",GOLD)]:
...
cells = {
 (0,0):["Order intake & Book-to-bill","Backlog (mesi di copertura)","Win rate / hit rate offerte","Aftermarket share %","Mix applicazioni & geografie"],
 (0,1):["Margine di offerta per commessa","% ordini sopra soglia margine","Pricing waterfall / discount leakage","Var. Prezzo / Volume / Mix","Margine aftermarket vs nuovo"],
 (0,2):["% acconto all'ordine","Aderenza milestone SAL","DSO clienti","Acconti ÷ WIP","Penali / performance bond"],
 (1,0):["Saturazione capacità (fabbrica & eng.)","On-Time Delivery (OTD)","Lead time order-to-delivery","Throughput (impianti completati)","NPD / time-to-market"],
 (1,1):["EAC vs budget / margine-a-finire","Produttività (ore std vs eff.) / OEE","Scrap · give-away · material yield","Material cost ratio & PPV","Costi garanzia · ricavi/FTE"],
 (1,2):["DIO magazzino (gg)","WIP unbilled & over/under-billing","DPO fornitori","Cash Conversion Cycle","Capex vs piano · FCF conversion"],
}
...
para(tfb,[("Lente captive: ",True,BLUE,13),("% infragruppo (acquisti/vendite) & margine stand-alone — per misurare la redditività reale e preparare un eventuale carve-out.",False,INK,13)],first=True,after=3)
para(tfb,[("CFO cockpit (10): ",True,BLUE,12.5),("order intake · book-to-bill · backlog · aftermarket % · EBITDA norm. · margine-a-finire · OTD · CCC · acconti÷WIP · FCF conversion.",False,"333333",12.5)],after=0)
